...
original_tar = glob.glob("/home/angelo/projects/dcm/dcmFiles/*.tgz")
os.chdir("/home/angelo/projects/dcm/dcmFiles/")
tar = tarfile.open(os.path.basename(original_tar[0]))
tar.extractall()
tar.close()
os.chdir("/home/angelo/projects/dcm/dcmFiles/tmp/.var.log-ro/")

alarms = (glob.glob("/home/angelo/projects/dcm/dcmFiles/tmp/.var.log-ro/alarms*.gz"))
with gzip.open(alarms[0], 'rb') as f_in:
    with open("/home/angelo/projects/dcm/dcmFiles/tmp/.var.log-ro/0-file", 'wb') as f_out:
        shutil.copyfileobj(f_in, f_out)

source = "/home/angelo/projects/dcm/dcmFiles/tmp/.var.log-ro/"
destination = "/home/angelo/projects/dcm/dcmFiles/unzipped_alarms"
extension = ".gz"
alarms = (glob.glob("/home/angelo/projects/dcm/dcmFiles/tmp/.var.log-ro/alarms*.gz"))
...
df_dejitter = df_dejitter.sort_values(['time'], ascending=[1])  # sorting by TS and time
df_dejitter['time'] = pd.to_datetime(df_dejitter['time']) # Converting time string to time time
df_dejitter['detail'] = df_dejitter['detail'].str.replace('TS=','') # Adjusting the TS column
df_dejitter['detail'] = df_dejitter['detail'].str.replace(':.+','') # Adjusting the TS column
...
# Preparing the plot of Dejitter alarms
xposition = list(df_dejitter['time'])
for xc in xposition:
    plt.axvline(x=xc, color='g', linestyle='-')

plt.title("BW Exceeded alarms / Green lines show Dejitter Buffer Resets for OIT multicast")
